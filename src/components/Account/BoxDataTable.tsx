import { Table, Box, Badge, Icon, Flex } from '@chakra-ui/react'
...
export function BoxDataTable({ logContent }: BoxDataTableProps) {
    ...
    return (
        <Box mt={4} rounded="lg" bg={bgColor} boxShadow="lg" overflow="auto" maxHeight="80vh">
            <Table.Root size="sm" style={{ maxWidth: '80%' } } border="1px solid gray" minWidth="max-content">
                <Table.Caption></Table.Caption>
                <Table.Header position="sticky" top={0} zIndex={1} backgroundColor={headerBgColor}>
                    <Table.Row>
                        <Table.Cell rowSpan={2} outline="1px solid gray" p={1} position="sticky" left={0} bg={headerBgColor} zIndex={2}>
                            用户名
                        </Table.Cell>
                        {infoColumns.includes('UID') && (
                            <Table.Cell rowSpan={2} border="1px solid gray" p={1} bg={headerBgColor}>
                                UID
                            </Table.Cell>
                        )}
                        ...
                        {allUnitIds.map((unitId, unitIndex) => {
                            ...
                            return (
                                <Table.Cell key={unitId} colSpan={unitAttributes.length} textAlign="center" bg={bgColorAlt} border="1px solid gray" p={1}>
                                    {unitName}
                                </Table.Cell>
                            )
                        })}
                            ...
                            unitAttributes.map((attr) => {
                                ...
                                return (
                                    <Table.Cell 
                                        ...
                                        p={1}
                                        onClick={() => handleSort(unitId, attr.id)}
                                    >
                ...
                <Table.Body>
                    {sortedUserUnitMaps.map(({ user, unitMap }) => (
                        <Table.Row key={user.uid}>
                            ...
                            {infoColumns.includes('UID') && (
                                <Table.Cell border="1px solid gray" p={1} bg={headerBgColor}>
                                    {user.uid}
                                </Table.Cell>
                            )}
                            {infoColumns.includes('数据时间') && (
                            ...
                            {allUnitIds.map((unitId, unitIndex) => {
                                ...
                                if (!unit || !unit.owned) {
                                    ...
                                    return unitAttributes.map((attr) => (
                                        <Table.Cell key={`${unitId}-${attr.id}`} textAlign="center" bg={bgColorAlt} opacity={0.5} border="1px solid gray" p={1}>
                                            -
                                        </Table.Cell>
                                    ))
                                }
                                ...
                                return unitAttributes.map((attr) => {
                                    ...
                                    if (attrId === 'rarity') {
                                        return (
                                            <Table.Cell key={`${unitId}-${attrId}`} textAlign="center" bg={bgColorAlt} border="1px solid gray" p={1}>
                                                <Badge>{value}★</Badge>
                                            </Table.Cell>
                                        )
                                    }
                                    ...
                                    return (
                                        <Table.Cell key={`${unitId}-${attrId}`} textAlign="center" fontSize="xs" bg={bgColorAlt} border="1px solid gray" p={1}>
                                            {value}
                                        </Table.Cell>
                                    )
                                })
