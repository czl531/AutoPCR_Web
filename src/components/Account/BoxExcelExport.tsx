...
export function BoxExcelExport({ logContent, fileName = 'box_data' }: ExcelExportProps) {
    const createCell = (value: string | number, align = 'center'): CellValue => ({ v: value, t: 's', s: { alignment: { horizontal: align } } });

    const showToast = (title: string, desc: string, status: 'error' | 'success') => {
        toaster.create({
            title,
            description: desc,
            type: status,
    ...
    const handleExport = () => {
        ...
        try {
            ...
            const columnWidths = [
                ...
                ...Array<number>(UnitIds.length * 13).fill(0).map(() => ({ wch: 8 }))
            ];
    ...
    return (
        <>
            <Button  colorScheme="teal" size="sm" onClick={handleExport} disabled={!logContent}>
                {<FiDownload />} 导出Excel
            </Button>
            <Toaster />
        </>
